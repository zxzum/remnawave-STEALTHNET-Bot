...
- [ ] **Step 1: Verify the worktree is clean and on the integration branch**
...
- [ ] **Step 2: Inspect each branch diff and run static whitespace checks**
...
- [ ] **Step 1: Merge the gifts branch without resolving unrelated files**

```bash
rtk git merge --no-ff codex/subscription-gifts -m "merge: subscription gifts"
```

- [ ] **Step 2: Run gift service tests and inspect the changed transaction boundaries**
...
- [ ] **Step 1: Merge cleanup and run its test**
...
- [ ] **Step 2: Inspect migration CLI defaults and dry-run behavior**
...
- [ ] **Step 3: Merge reissue and resolve only actual conflicts**
...
- [ ] **Step 4: Run reissue tests and verify the UUID assertions**
...
- [ ] **Step 1: Merge admin conversion and run its contract test**
...
- [ ] **Step 2: Merge client conversion/UI and resolve shared API files by union**
...
- [ ] **Step 3: Run conversion tests and compile-time checks**
...
- [ ] **Step 1: Write the failing whole-flow test before changing production code**
...
- [ ] **Step 2: Run the new test and confirm it fails for a missing integration behavior**
...
- [ ] **Step 3: Implement the minimum root-cause fix and rerun the focused test**
...
- [ ] **Step 4: Add race and retry assertions**
...
- [ ] **Step 1: Run both migration dry-runs without apply**

```bash
cd backend
rtk npm run migrate:composite-to-single -- --username vssarovv --dry-run
rtk npm run migrate:composite-to-single -- --username sallyqx --dry-run
cd ..
```

Expected: both commands report planned changes only and neither command contains or executes `--apply`.
...
- [ ] **Step 3: Run the security diff review for `4b6e759..HEAD`**
...
- [ ] **Step 5: Re-read the checklist against branch history and working tree**
