 ...
 *
 * Ограничения (спецификация §3.2): агрегированный лимит на порт управляемого
 * inbound; qdisc clsact + flower/police фильтры с собственными pref/handle;
 * root qdisc и firewall не трогаем; IPv4+IPv6, TCP+UDP.
 */

export const TC_SCRIPT_PATH = "/usr/local/sbin/lazeika-only-tc";
export const TC_UNIT_NAME = "lazeika-only-tc.service";
/** Собственные pref — чужие фильтры не трогаем, только свои del+replace'им. */
...
/**
 * Скрипт применения лимита. Идемпотентный: сперва удаляет собственные pref,
 * затем ставит заново с актуальным rate. Root qdisc не изменяется.
 * 8 фильтров = {ip,ipv6} x {ingress,egress} x {tcp,udp}, ingress → dst_port, egress → src_port.
 */
export function buildTcScript(args: { iface: string; port: number; speedMbit: number }): string {
  ...
  const lines: string[] = [
    ...
    "",
    `# Удаляем только СВОИ фильтры (pref ${TC_PREF_BASE}..${TC_PREF_BASE + 7}), затем ставим заново.`,
  ];
  let seq = 0;
  for (const protocol of ["ip", "ipv6"] as const) {
    for (const direction of ["ingress", "egress"] as const) {
      for (const ipProto of ["tcp", "udp"] as const) {
        ...
        lines.push(`tc filter del dev "$IFACE" ${direction} pref ${pref} 2>/dev/null || true`);
        lines.push(
          `tc filter replace dev "$IFACE" ${direction} pref ${pref} handle 1:${seq}0 protocol ${protocol} flower ip_proto ${ipProto} ${match} "$PORT" action police rate ${args.speedMbit}mbit burst 256kb drop`,
        );
      }
    }
  }
  lines.push("", 'echo "lazeika-only-tc applied on $IFACE port $PORT at ' + args.speedMbit + 'mbit"');
  return lines.join("\n") + "\n";
}
...
export function buildTcUnit(): string {
  return [
    "[Unit]",
    "Description=Lazeika-Only tc rate limit (managed by panel)",
