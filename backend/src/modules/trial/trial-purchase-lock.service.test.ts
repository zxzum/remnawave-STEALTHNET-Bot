...
test("an admin-granted subscription also consumes the trial", () => {
  assert.equal(isAdminGrantPayment({ status: "PAID", provider: "admin_grant" }), true);
  assert.equal(isAdminGrantPayment({ status: "PAID", metadata: JSON.stringify({ kind: "admin_grant" }) }), true);
  assert.equal(isAdminGrantPayment({ status: "PENDING", provider: "admin_grant" }), false);
  assert.equal(isAdminGrantPayment({ status: "PAID", provider: "balance" }), false);
});
